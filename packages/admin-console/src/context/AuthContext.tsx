...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  const [auth, setAuth] = useState<AuthState>(() => {
    try {
      const stored = localStorage.getItem('civicmind_admin_session');
  ...
  useEffect(() => {
    if (auth.token) localStorage.setItem('civicmind_admin_session', JSON.stringify(auth));
    else localStorage.removeItem('civicmind_admin_session');
  }, [auth]);
