...
export default function LoginScreen() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.role !== 'authority' && data.role !== 'admin') {
          setError('Access denied. Authority portal requires staff credentials.');
        } else {
  ...
  const handleDemoLogin = () => {
    login('demo-authority-token', {
      access_token: 'demo-authority-token',
      user_id: 'auth-001',
      role: 'authority',
      department_id: 'dept-bbmp-roads',
      jurisdiction_scope: ['ward-101-indiranagar'],
    });
    navigate('/dashboard');
  };
      ...
      <div style={{ 
        ...
        <div style={{ maxWidth: '480px' }}>
          ...
          <h1 style={{ fontSize: '36px', fontWeight: 800, marginBottom: '16px', lineHeight: 1.2 }}>
            Manage civic issues for your department
          </h1>
          <p style={{ fontSize: '18px', color: 'rgba(255,255,255,0.9)', marginBottom: '48px', lineHeight: 1.5 }}>
            Sign in to access your operational dashboard, review incoming reports, and update resolution statuses on the ground.
          </p>
          
          <div style={{ display: 'flex', gap: '16px' }}>
            <div style={{ background: 'rgba(0,0,0,0.1)', padding: '16px', borderRadius: '12px', flex: 1 }}>
              <div style={{ fontSize: '24px', fontWeight: 700, marginBottom: '4px' }}>198</div>
              <div style={{ fontSize: '14px', color: 'rgba(255,255,255,0.8)' }}>Wards Covered</div>
            </div>
            <div style={{ background: 'rgba(0,0,0,0.1)', padding: '16px', borderRadius: '12px', flex: 1 }}>
              <div style={{ fontSize: '24px', fontWeight: 700, marginBottom: '4px' }}>48h</div>
              <div style={{ fontSize: '14px', color: 'rgba(255,255,255,0.8)' }}>Avg. Resolution</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ 
        ...
        <div style={{ width: '100%', maxWidth: '400px' }}>
          <div style={{ marginBottom: '32px' }}>
            <h2 style={{ fontSize: '28px', fontWeight: 700, color: 'var(--color-text-primary, #1e293b)', marginBottom: '8px' }}>Authority Portal</h2>
            <p style={{ color: 'var(--color-text-secondary, #64748b)' }}>Enter your staff credentials to continue</p>
          </div>

          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div>
              <label style={{ display: 'block', fontSize: '14px', fontWeight: 600, color: 'var(--color-text-secondary)', marginBottom: '8px' }}>Official Email</label>
              <input 
                type="email" 
                required
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder="officer.bescom@civicmind.gov"
