 ...
 */
import { IssueStatus, IssueSeverity, IssueCategory, VALID_STATUS_TRANSITIONS, CLASSIFICATION_AUTO_CONFIRM_THRESHOLD, DUPLICATE_AUTO_MERGE_THRESHOLD, DUPLICATE_CONFIRM_THRESHOLD, SLA_AT_RISK_PERCENTAGE, SLARisk, } from './types/enums.js';
// ─── State machine ────────────────────────────────────────────────────────────
/**
 ...
 */
export function requiresCitizenConfirmation(categoryConfidence) {
 ...
 */
export function getDuplicateAction(confidence) {
    if (confidence >= DUPLICATE_AUTO_MERGE_THRESHOLD)
        return 'auto_merge';
    if (confidence >= DUPLICATE_CONFIRM_THRESHOLD)
 ...
 */
export function computeSLADeadline(routedAt, slaHours) {
 ...
 */
export function computeSLARisk(slaDeadline, now = new Date()) {
 ...
 */
export function computeSLARiskFull(routedAt, slaDeadline, now = new Date()) {
 ...
 */
export function geodesicDistanceMeters(lat1, lng1, lat2, lng2) {
 ...
 */
export function isWithinServiceArea(lat, lng, bbox) {
    return (lat >= bbox.lat_min &&
        lat <= bbox.lat_max &&
        lng >= bbox.lng_min &&
        lng <= bbox.lng_max);
}
