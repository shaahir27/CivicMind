...
5. **`packages/backend` (Backend)** — A Node.js/Express server housing the **Agent Orchestrator** (powered by `@google/genai` & Gemini 3.1 Flash-Lite) that coordinates downstream agents, manages issue state, and handles API requests.
...
- A central Express server responsible for:
  ...
  - Sequencing agent invocations via `@google/genai` (Gemini 3.1 Flash-Lite).
...
|---|---|---|
| `@google/genai` (Gemini 3.1 Flash-Lite) | Orchestrator reasoning, Vision analysis | Handles both text reasoning and multimodal image analysis |
